...
const CartForm = ({ location, getLocation, user, cartItem, updateQuantity, deleteItem, navigate, totalPrice }) => (
  <div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-2 md:gap-20'>
        <div className='bg-gray-100 rounded-md p-7 mt-4 space-y-2'>
          ...
          <div className='flex flex-col space-y-1'>
            ...
            <input type="text" placeholder='Enter your name' className='p-2 rounded-md' value={clerkEnabled ? (user?.fullName || '') : ''} readOnly={!clerkEnabled}/>
          </div>
          <div className='flex flex-col space-y-1'>
...
const ClerkCart = ({ location, getLocation }) => {
  const { user } = useUser()
...
const Cart = ({location, getLocation}) => {
  ...
  if (clerkEnabled) {
    return <ClerkCart location={location} getLocation={getLocation} />
  }

  return (
    <div className='mt-10 max-w-6xl mx-auto mb-5 px-4 md:px-0'>
      {cartItem.length > 0 ? <CartForm location={location} getLocation={getLocation} user={null} cartItem={cartItem} updateQuantity={updateQuantity} deleteItem={deleteItem} navigate={navigate} totalPrice={totalPrice} /> : <div className='flex flex-col gap-3 justify-center items-center h-[600px]'>
        <h1 className='text-red-500/80 font-bold text-5xl text-muted'>Oh no! Your cart is empty</h1>
        <img src={emptyCart} alt="" className='w-[400px]'/>
        <button onClick={()=>navigate('/products')} className='bg-red-500 text-white px-3 py-2 rounded-md cursor-pointer '>Continue Shopping</button>
      </div>}
